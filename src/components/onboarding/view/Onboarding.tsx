...
export default function Onboarding({ onComplete }: OnboardingProps) {
  ...
  const handleNextStep = async () => {
    ...
    if (!gitName.trim() || !gitEmail.trim()) {
      setErrorMessage(t('onboarding.errors.nameAndEmailRequired'));
      return;
    }

    if (!gitEmailPattern.test(gitEmail)) {
      setErrorMessage(t('onboarding.errors.invalidEmail'));
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const message = await readErrorMessageFromResponse(response, t('onboarding.errors.saveGitConfig'));
        throw new Error(message);
      }

      setCurrentStep((previous) => previous + 1);
    } catch (caughtError) {
      setErrorMessage(caughtError instanceof Error ? caughtError.message : t('onboarding.errors.saveGitConfig'));
    } finally {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const message = await readErrorMessageFromResponse(response, t('onboarding.errors.completeOnboarding'));
        throw new Error(message);
      }

      await onComplete?.();
    } catch (caughtError) {
      setErrorMessage(caughtError instanceof Error ? caughtError.message : t('onboarding.errors.completeOnboarding'));
    } finally {
      setIsSubmitting(false);
    }
  };
